...
function Execute-BackupFolders
{
    ...
    if ($useVSS) {
        ...
        $backupModeBool = [bool]$UseBackupMode
        Execute-BackupWithRobocopy -Folders $Folders -BackupTempLocation $BackupTempLocation -BackupSetsLocation $BackupSetsLocation -LogFile $LogFile -Password $Password -Compress $Compress -Encrypt $Encrypt -RetryCount $RetryCount -UseBackupMode $backupModeBool
    }
}
...
function Execute-BackupWithRobocopy
{
    ...
    param (
        ...
        [boolean]$Encrypt,
        [int]$RetryCount = 3,
        [boolean]$UseBackupMode = $true  # Исправлено: boolean вместо switch
    )
    ...
    # Проверяем права администратора для режима /B
    $isAdmin = ([Security.Principal.WindowsPrincipal] [Security.Principal.WindowsIdentity]::GetCurrent()).IsInRole([Security.Principal.WindowsBuiltInRole] "Administrator")
    if ($UseBackupMode -and -not $isAdmin) {
        Write-Warning "Backup mode (/B) requires administrator privileges. Switching to standard mode."
        $UseBackupMode = $false
    }
    ...
    foreach ($folder in $Folders) {
        ...
        try {
            ...
            # Добавляем /B если включен режим резервного копирования и есть права админа
            if ($UseBackupMode -and $isAdmin) {
                $robocopyArgs += "/B"
                Write-Host "$(get-date -format 'dd.MM.yy HH:mm:ss'): Using Robocopy backup mode (/B) for $folder"
            } else {
                Write-Host "$(get-date -format 'dd.MM.yy HH:mm:ss'): Using Robocopy standard mode for $folder"
            }
            ...
            # Анализируем код возврата Robocopy
            $exitCode = $robocopyResult.ExitCode
            Write-Host "$(get-date -format 'dd.MM.yy HH:mm:ss'): Robocopy completed with exit code: $exitCode"
            
            # Коды возврата Robocopy:
            # 0 = Успех, никаких файлов не скопировано
            # 1 = Успех, файлы скопированы
            # 2-7 = Частичный успех с предупреждениями
            # 8+ = Ошибки
            
            if ($exitCode -le 7) {
                # Успешное или частично успешное выполнение
                if ($exitCode -eq 0) {
                    Write-Host "$(get-date -format 'dd.MM.yy HH:mm:ss'): No changes detected in $folder"
                } else {
                    Write-Host "$(get-date -format 'dd.MM.yy HH:mm:ss'): Successfully synchronized $folder"
                }
                ...
                if ($LogFile) {
                    "$(get-date -format 'dd.MM.yy HH:mm:ss'): Robocopy failed for $folder with exit code $exitCode" | Out-File $LogFile -Encoding unicode -Append
                }
            }
            ...
            if ($LogFile) {
                "$(get-date -format 'dd.MM.yy HH:mm:ss'): Robocopy backup failed for $folder - $errorMessage" | Out-File $LogFile -Encoding unicode -Append
            }
        }
    }
}
...
# Запуск бэкапа
Execute-BackupFolders -Folders 'D:\!Disp', 'C:\Users' -BackupTempLocation 'F:\TMP' -BackupSetsLocation 'F:\!Backup' -ForceRobocopy -UseBackupMode -LogFile 'F:\!Backup\logs.log' -Compress $true
...
# 1. Автоматический выбор с режимом /B (по умолчанию)
#Execute-BackupFolders -Folders 'D:\!Disp', 'C:\Users' -BackupTempLocation 'F:\TMP' -BackupSetsLocation 'F:\!Backup' -LogFile 'F:\!Backup\logs.log' -Compress $true

# 2. Принудительно Robocopy с режимом /B
#Execute-BackupFolders -Folders 'D:\!Disp', 'C:\Users' -BackupTempLocation 'F:\TMP' -BackupSetsLocation 'F:\!Backup' -ForceRobocopy -UseBackupMode -Compress $true

# 3. Robocopy без режима /B (если нет прав админа)
#Execute-BackupFolders -Folders 'D:\!Disp', 'C:\Users' -BackupTempLocation 'F:\TMP' -BackupSetsLocation 'F:\!Backup' -ForceRobocopy -UseBackupMode:$false -Compress $true
