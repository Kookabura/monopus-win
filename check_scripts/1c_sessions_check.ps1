...
try
{
    ...
    $result = Start-Job -ArgumentList @($server, $log_file, $platform1c_obj) -ScriptBlock {
        ...
        foreach ($session in $sessions)
        {
            ...
            if ($session.AppID -match "BackgroundJob|SrvrConsole")
            {
                $BackgroundJob++
            }
            else
            {
                if ($unicUser -notcontains $session.userName)
                {
...
$output = "1c_sessions_check.$($states_text[$state])::all_sessions_count==$($result.SessionCount)__unic_user==$($result.UniqueUser)__background_job==$($result.BackgroundJob) | all_sessions_count=$($result.SessionCount);;;; unic_user=$($result.UniqueUser);;;; background_job=$($result.BackgroundJob);;;;"
